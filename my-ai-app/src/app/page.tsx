...
async function sendToDB(user_email, mess, recording, text, copies, timers, objectId) {
  ...
  try {
    ...
    const messageResponse = await axios.post("https://gptwriting.cs.vt.edu/research/save_messages", {
      user_id: objectId,
      email: user_email,
      mess_array: mess,
      copy_array: copies,
    ...
    const recordingResponse = await axios.post("https://gptwriting.cs.vt.edu/research/save_recording", {
      user_id: objectId,
      email: user_email,
...
async function sendEmail(user_email, objectId, startTime)
{
  const timestamp = new Date().toISOString();
  try {
    const emailResponse = await axios.post("https://gptwriting.cs.vt.edu/research/save_email", {
      user_id: objectId,
      email: user_email,
      start: timestamp,
    });
